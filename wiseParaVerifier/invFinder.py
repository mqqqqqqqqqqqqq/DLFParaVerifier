...
class ConstructF():
    ...
    def smtFormula(self):
        ...
        while checkSat:
            ...
            start_result, _ = self.call_NuSMV(current_solutions, self.protocol_name)
            # largerpn = "largerN" + "_" + self.protocol_name
            # start_result, _ = self.call_BMC(current_solutions, largerpn)
            # start_result, _ = self.call_cmurphi(current_solutions)
            # print("current_solutions: ", current_solutions, start_result)
            ...
            if start_result:
                ...
                if USE_HEURISTIC_GENERALIZE == True:
                    ...
                    if len(join_list) > 0 :
                        if current_solutions:
                            ...
                            # print("join_current_inv: ", join_current_inv, join_inv4smt)
                            # print(f"invariant \"{self.name}_{self.invnums}\"")
                            # print(str(join_inv4smt))
                ...
                if USE_HEURISTIC_GENERALIZE == False or (len(join_list) == 0 or (join_current_inv == None and join_inv4smt == None)):
                    ...
                    if current_solutions:
                        ...
                        # print(f"invariant \"{self.name}_{self.invnums}\"")
                        # print(str(inv4smt))
                        # print("current_inv: ", current_inv, inv4smt)
            ...
            checkSat = self.solver.check() == sat
            # print("solver: ", self.solver.check())
            e_smt = time.time()
            global t_smt
            t_smt += e_smt - s_smt
    ...
    def getallSymmetryInvs_partition(self, ori_inv, subsidiary = False):
        ...
        # print("flpartitionsRecord: ", flpartitionsRecord)
        ...
        if EXISTENTIAL_QUANTIFICATION == True:
            for var,key in digitdict.items():
                if len(key) == murphi.const_map[var]:
                    ...
                    # print("smv_result: ", self.call_BMC(statements,largerpn)[0])
                    if not self.call_BMC(statements,largerpn)[0]:
                    # if not self.call_cmurphi(statements)[0]:
            ...
            for existsType in existsTypes:
                ...
                for combination in value_combinations:
                    ...
                    for symm_map in symm_maps:
                        ...
                        # largerpn = "largerN" + "_" + self.protocol_name
                        # checkpass = self.call_BMC(new_statements,largerpn)[0]
                        # checkpass = self.call_cmurphi(new_statements)[0]
                        if checkpass == False:
                            symmFlag = False
                            break
                    if symmFlag == True:
        ...
        # print("symminvs: ", symminvs)
        # print("exfolparainvs: ", exfolparainvs)
        # print("allparainvs: ", allparainvs)
        return symminvs
    ...
    def call_Check(self, inv_list):
        ...
        # largerpn = "largerN" + "_" + self.protocol_name
        # pass_check, can_inv = self.call_BMC(inv_list, largerpn)
        ...
        while n <= len(diff_list):
            ...
            for sublist in n_length_sublists:
                ...
                if validFlag:
                    ...
                    # largerpn = "largerN" + "_" + self.protocol_name
                    # pass_check, can_inv = self.call_BMC(ready_to_check, largerpn)
                    # pass_check, can_inv = self.call_cmurphi(ready_to_check)
                    # print("call_Check_join_increase: ", pass_check, can_inv)
                if pass_check:
        ...
        for n in range(1, inv_list_length+1):
            ...
            for sublist in n_length_sublists:
                ...
                if validFlag:
                    ...
                    # pass_check, can_inv = self.call_BMC(ready_to_check, largerpn)
                    # pass_check, can_inv = self.call_cmurphi(ready_to_check)
                    # print("call_Check_increase: ", pass_check, can_inv)
                if pass_check:
    ...
    def call_NuSMV(self, inv_list, protocol_name):
        ...
        # print("inv_to_check: ", protocol_name, inv_to_check)
    ...
    def call_cmurphi(self, inv_list):
        ...
        self.appendInv_and_save(self.inv_path + "_withoutInv.m", new_inv, self.inv_path + "_withInv.m")

        cmurphi_path = '../cmurphi_LS'
        # /home/lyj238/Mqian/smt_dl/cmurphi_LS
        # file = '../protocols/mutualEx/mutualEx.m'
        file = self.inv_path
        process1 = subprocess.Popen("{0}/src/mu {1}_withInv.m".format(cmurphi_path, file), shell=True,
        ...
        command2 = "g++ -ggdb -o {0}_withInv.o {0}_withInv.cpp -I {1}/include -lm".format(file, cmurphi_path)
        process2 = subprocess.Popen(command2, shell=True,
                                    stdout=subprocess.PIPE,
                                    stderr=subprocess.PIPE)
        process2.communicate()
        if not os.path.exists("./{0}_withInv.o".format(file)): raise FileExistsError

        process3 = subprocess.Popen("./{0}_withInv.o".format(file), shell=True,
                                    stdout=subprocess.PIPE,
                                    stderr=subprocess.PIPE)
        ...
        os.remove('%s_withInv.cpp' % file)
        os.remove('%s_withInv.o' % file)
        # os.remove('%s_withInv.m' % file)
...
def Verification(protocol_path):
    ...
    # with open(protocol_path + "_node2.smv", "w") as f:
    #     f.write(str(smv_content))

    # using smv as checker
    smv_content = ""
    with open(protocol_path + "_node2.smv", "r") as file:
...
if __name__ == "__main__":
    ...
    protocol_path = "./protocols/flashNodata/flash_nodata_cub"
    # protocol_path = "./protocols/germanNoData/german_withoutData"
    # protocol_path = "./protocols/mutualEx/mutualEx"
    # protocol_path = "./protocols/MOESI/Mesi"
    # sys.stdout = open(protocol_path + '_debug.log', 'w')
    # ivyselect = "#lang ivy1.7"
    # trans2IVY(protocol_path, ivyselect)
