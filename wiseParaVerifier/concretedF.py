import sys
sys.path.append('./')
sys.path.append('/home/lyj238/Mqian/WiseParaVerifier/wiseParaverifier')
import murphi
...
class GetSMTformula:
    ...
    def getDlInv(self):
        ...
        for inv, rules in self.rule_var_ins.items():
            ...
            for rule, rule_vars in rules.items():
                ...
                # print("self.rule_var_list: ", self.rule_var_list)
                ...
                for ins_permutation in ins_permutations:
                    ...
                    guard_dp = copy.deepcopy(self.rule_para[rule]["guard"])
                    # print(guard_dp)
                    # noins_guard_dp = murphi.NegExpr(copy.deepcopy(self.rule_para[rule]["guard"]))
                    # if noins_guard_dp not in noins_guard:
                    #     noins_guard.append(noins_guard_dp)
                    guard_vars = []
        ...
        # print(noins_guard)
        # shutil.copyfile(self.parse_name + ".m", self.parse_name + "_deadlock.m")
        # noins_guard_formula = murphi.NegExpr(self.join_statements(noins_guard))
        # with open(self.parse_name + "_deadlock.m", "a") as file:
        #     file.write("\n\n")
        #     file.write("invariant \"deadlock\" \n")
        #     file.write("forall i : NODE do \n")
        #     file.write(murphi.indent(str(noins_guard_formula), 2) + "\n")
        #     file.write("end; \n")
    ...
    def get_deadlock(self):
        self.getDlInv()
        self.invariant_ins[self.parse_name] = self.all_guard
        self.getInv_2()

    def get_deadlock_free(self):
        self.getDlInv()
        self.invariant_ins[self.parse_name] = murphi.NegExpr(self.all_guard)
        self.getInv_2()
    ...
    def getInv_2(self):
        ...
        if self.invariant_ins:
            ...
            self.invariant_ins[self.parse_name] = None

        for key in self.invariant_ins.keys():
            inv_name = key
        # print("inv_name: ", inv_name)
        inv = self.invariant_ins[self.parse_name]
        # print("inv: ", inv)
        # 带参形式的实例化
...
if __name__ == "__main__":
    ...
    parse_name = "./protocols/flashNodata/flash_nodata_cub"
    ...
    GetSMTformula(parse_name=parse_name).getDlInv()
